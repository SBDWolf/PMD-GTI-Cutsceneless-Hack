  ...
  if symbol == "@EXIT" then
    ...
    local __LB_SCOPE = function()
      ...
      function __scopeFuncTbl.CELECT_EVENT01()
        WINDOW:SysMsg(922318245)
        local __labelCnt = 0
        local __labelTbl = {}
        WINDOW:SelectStart()
        WINDOW:SelectChain(803357924, __labelCnt)
        __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
        __labelCnt = __labelCnt + 1
        WINDOW:SelectChain(80680743, __labelCnt)
    ...
    do
      local __LB_SCOPE = function()
        ...
        function __scopeFuncTbl.CELECT_EVENT02()
          WINDOW:SysMsg(500442726)
          local __labelCnt = 0
          local __labelTbl = {}
          WINDOW:SelectStart()
          WINDOW:SelectChain(1385541793, __labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
          __labelCnt = __labelCnt + 1
          WINDOW:SelectChain(1267630560, __labelCnt)
          WINDOW:DefaultCursor(__labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_NO
          __labelCnt = __labelCnt + 1
          local __res = WINDOW:SelectEnd(MENU_SELECT_MODE.ENABLE_CANCEL)
