  ...
  if symbol == "@EXIT" then
    ...
    local __LB_SCOPE = function()
      ...
      function __scopeFuncTbl.CELECT_EVENT01()
        WINDOW:SysMsg(-1320947264)
        local __labelCnt = 0
        local __labelTbl = {}
        WINDOW:SelectStart()
        WINDOW:SelectChain(-1470578559, __labelCnt)
        __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
        __labelCnt = __labelCnt + 1
        WINDOW:SelectChain(-2089446590, __labelCnt)
    ...
    do
      local __LB_SCOPE = function()
        ...
        function __scopeFuncTbl.CELECT_EVENT02()
          WINDOW:SysMsg(-1704025597)
          local __labelCnt = 0
          local __labelTbl = {}
          WINDOW:SelectStart()
          WINDOW:SelectChain(-718326588, __labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
          __labelCnt = __labelCnt + 1
          WINDOW:SelectChain(-869005947, __labelCnt)
          WINDOW:DefaultCursor(__labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_NO
          __labelCnt = __labelCnt + 1
          local __res = WINDOW:SelectEnd(MENU_SELECT_MODE.ENABLE_CANCEL)
