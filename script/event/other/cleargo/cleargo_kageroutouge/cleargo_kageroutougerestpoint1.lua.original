  ...
  if symbol == "@EXIT" then
    ...
    local __LB_SCOPE = function()
      ...
      function __scopeFuncTbl.CELECT_EVENT01()
        WINDOW:SysMsg(433162210)
        local __labelCnt = 0
        local __labelTbl = {}
        WINDOW:SelectStart()
        WINDOW:SelectChain(13285027, __labelCnt)
        __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
        __labelCnt = __labelCnt + 1
        WINDOW:SelectChain(736617824, __labelCnt)
    ...
    do
      local __LB_SCOPE = function()
        ...
        function __scopeFuncTbl.CELECT_EVENT02()
          WINDOW:SysMsg(855430177)
          local __labelCnt = 0
          local __labelTbl = {}
          WINDOW:SelectStart()
          WINDOW:SelectChain(2109555430, __labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
          __labelCnt = __labelCnt + 1
          WINDOW:SelectChain(1688630183, __labelCnt)
          WINDOW:DefaultCursor(__labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_NO
          __labelCnt = __labelCnt + 1
          local __res = WINDOW:SelectEnd(MENU_SELECT_MODE.ENABLE_CANCEL)
