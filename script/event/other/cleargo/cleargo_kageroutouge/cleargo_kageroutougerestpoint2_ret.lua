  ...
  if symbol == "@EXIT" then
    ...
    local __LB_SCOPE = function()
      ...
      function __scopeFuncTbl.CELECT_EVENT01()
        WINDOW:SysMsg(-4501689)
        local __labelCnt = 0
        local __labelTbl = {}
        WINDOW:SelectStart()
        WINDOW:SelectChain(-425689594, __labelCnt)
        __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
        __labelCnt = __labelCnt + 1
        WINDOW:SelectChain(-846385723, __labelCnt)
    ...
    do
      local __LB_SCOPE = function()
        ...
        function __scopeFuncTbl.CELECT_EVENT02()
          WINDOW:SysMsg(-728359804)
          local __labelCnt = 0
          local __labelTbl = {}
          WINDOW:SelectStart()
          WINDOW:SelectChain(-1680373181, __labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
          __labelCnt = __labelCnt + 1
          WINDOW:SelectChain(-2100511998, __labelCnt)
          WINDOW:DefaultCursor(__labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_NO
          __labelCnt = __labelCnt + 1
          local __res = WINDOW:SelectEnd(MENU_SELECT_MODE.ENABLE_CANCEL)
