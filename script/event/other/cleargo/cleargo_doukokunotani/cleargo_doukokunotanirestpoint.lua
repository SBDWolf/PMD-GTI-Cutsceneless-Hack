  ...
  if symbol == "@EXIT" then
    ...
    local __LB_SCOPE = function()
      ...
      function __scopeFuncTbl.CELECT_EVENT01()
        WINDOW:SysMsg(-229014671)
        local __labelCnt = 0
        local __labelTbl = {}
        WINDOW:SelectStart()
        WINDOW:SelectChain(-347950544, __labelCnt)
        __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
        __labelCnt = __labelCnt + 1
        WINDOW:SelectChain(-1066409485, __labelCnt)
    ...
    do
      local __LB_SCOPE = function()
        ...
        function __scopeFuncTbl.CELECT_EVENT02()
          WINDOW:SysMsg(-646655822)
          local __labelCnt = 0
          local __labelTbl = {}
          WINDOW:SelectStart()
          WINDOW:SelectChain(-1774893451, __labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_YES
          __labelCnt = __labelCnt + 1
          WINDOW:SelectChain(-1892780236, __labelCnt)
          WINDOW:DefaultCursor(__labelCnt)
          __labelTbl[__labelCnt] = __scopeFuncTbl.LABEL_NO
          __labelCnt = __labelCnt + 1
          local __res = WINDOW:SelectEnd(MENU_SELECT_MODE.ENABLE_CANCEL)
